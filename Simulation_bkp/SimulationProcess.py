...
from Simulation_bkp.GlobalConfig import *
...
def run(locs, p_size, sigma, sim_time, gateway_location, num_nodes, transmission_rate, confirmed_messages, adr):
    ...
    data_mean_nodes = Node.get_mean_simulation_data_frame(nodes, name=sigma) / num_nodes

    data_gateway = gateway.get_simulation_data(name=sigma) / num_nodes
    data_air_interface = air_interface.get_simulation_data(name=sigma) / num_nodes
